  ...
  : update_freq_(update_freq)
{
  ...
  rsc_car_port.set_port("/dev/ttyUSB0");
  rsc_car_port.set_baudRate(9600);
  rsc_car_port.set_dataBits(8);
  rsc_car_port.set_stopBit(1);
  rsc_car_port.set_parity(false);
  rsc_car_port.set_hardwareFlowControl(false);
}
...
void HwTmIntf::update()
{
  ...
  rsc_car_port.writeData(ss_cmd_vel_.str());
  ...
  rsc_car_port.writeData(ss_curr_vel_.str());
  hw_data_.act_curr_pos_[0] = ((float)atoi((rsc_car_port.readData()).c_str()) / ENC_FULL) * (2 * PI);


  ss_curr_vel_.str("");
  ss_curr_vel_ << 2 << "pos\r"; // Left wheel (node 2)
  rsc_car_port.writeData(ss_curr_vel_.str());
  hw_data_.act_curr_pos_[1] = -((float)atoi((rsc_car_port.readData()).c_str()) / ENC_FULL) * (2 * PI);
